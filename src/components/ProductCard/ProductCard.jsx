...
export default function ProductCard({product}) {
  ...
  return (
    <Link to={`/products/${product.slug}` ?? ''} className="flex flex-col max-w-[370px] flex-wrap p-[16px] bg-[#081116] hover:ring-opacity-40 active:ring-5 active:ring-[#6247eb] hover:ring-4 active:ring-2 active:ring-opacity-90" >
         <img src={product?.imageUrl ?? ''} alt={product?.name ?? 'No Title'} className="block max-h-[250px] mb-5 object-cover" />
          <div className="flex flex-col gap-2">
              ...
              <div>
                  {product.stock <= 0 ? (
                    ...
                    <>
                       <p className="text-xl font-semibold text-center text-yellow-500">Almost Sold Out</p>
                        <Button type="button" className="inline-flex items-center justify-center gap-2 p-4 bg-[#000B58] text-center hover:bg-[#5969cf] text-white active:bg-[#4956ab]">
                  ...
                  <Button type="button" className="inline-flex items-center justify-center gap-2 p-4 bg-[#000B58] text-center hover:bg-[#5969cf] text-white active:bg-[#4956ab] mt-[28px]">
                      <FontAwesomeIcon icon={faCartShopping} className="mb-0" />
                      <span>Add to cart</span>
                    </Button>
